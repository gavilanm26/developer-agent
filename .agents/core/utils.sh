...
apply_global_templates() {
    local target_dir="$1"
    local global_tpl_dir="$AGENT_ROOT/.agents/.templates/global"
    if [ -d "$global_tpl_dir" ]; then
        ...
        # CORRECCIÓN: El find ahora ignora la carpeta .agents
        find "$target_dir" -path "*/.agents" -prune -o -name "*.tpl" -exec sh -c 'mv "$1" "${1%.tpl}"' _ {} \;
    fi
}