...
TPL_DIR="$ROOT_AGENT_DIR/.agents/.templates/templates-gateway-endpoint"
...
mkdir -p "$ROOT_AGENT_DIR/.agents/tmp"
...
if [ "$MODULE_EXISTS" = "false" ] || [ "$SKIP_QUALITY" = "true" ]; then
    ...
    if [ "$GW_MODE" == "rest" ]; then
        ...
        find "$ENDPOINT_DIR" -name "*.ts" -exec sh -c 'source "'"$ROOT_AGENT_DIR"'/.agents/core/utils.sh"; clean_graphql_artifacts "$1"' _ {} \;
    fi

    # Registro y Sincronización
    echo -e "${BLUE}🔗 Sincronizando módulo en AppModule...${NC}"
    bash "$ROOT_AGENT_DIR/.agents/core/sync-appmodule-endpoints.sh"
fi
...
if [ "$SKIP_QUALITY" = "false" ]; then
    ...
    while [ $ATTEMPT -lt $MAX_RETRIES ]; do
        ...
        if grep -q "FAIL" "$TEST_LOG" || ! grep -q "PASS" "$TEST_LOG"; then
            ...
            PROMPT_FILE="$ROOT_AGENT_DIR/.agents/tmp/fix_prompt.txt"
            cat <<EOF > "$PROMPT_FILE"
